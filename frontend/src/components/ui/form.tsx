...
const FormDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => {
    ...
    return <p ref={ref} id={formDescriptionId} className={cn("text-xs text-slate-400 mt-1", className)} {...props} />
  }
)
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    if (!body) return null
    return (
      <p ref={ref} id={formMessageId} className={cn("text-xs font-medium text-red-500 mt-1", className)} {...props}>
        {body}
      </p>
    )
  }
